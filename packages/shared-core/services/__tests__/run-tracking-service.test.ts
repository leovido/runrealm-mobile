...
import { RunPoint, RunTrackingService } from '../run-tracking-service';
...
describe('RunTrackingService', () => {
  ...
  describe('Starting a Run', () => {
    ...
    it('should throw error if run is already in progress', async () => {
      ...
      await expect(service.startRun()).rejects.toThrow('A run is already in progress');
    });

    it('should throw error if location service is not available', async () => {
      service.setLocationService(null as any);

      await expect(service.startRun()).rejects.toThrow('LocationService not available');
    });

    it('should throw error if location cannot be obtained', async () => {
      mockLocationService.getCurrentLocation.mockResolvedValue(null);

      await expect(service.startRun()).rejects.toThrow('Unable to get current location');
    });
  ...
  describe('GPS Point Processing', () => {
    ...
    it('should filter out points that are too close together', () => {
      ...
      if (eventHandler) {
        ...
        const pointCountAfterFirst = service.getCurrentRun()?.points.length || 0;
        eventHandler(locationInfo2);
        const pointCountAfterSecond = service.getCurrentRun()?.points.length || 0;
  ...
  describe('Smoothing Filter', () => {
    it('should apply smoothing to GPS points', () => {
      ...
      const smoothed = (service as any).applySmoothingFilter(lastPoint, newPoint);
  ...
  describe('Event Emission', () => {
    ...
    it('should emit run:pointAdded event when point is added', () => {
      ...
      const pointAddedCalls = (mockEventBus.emit as jest.Mock).mock.calls.filter(
